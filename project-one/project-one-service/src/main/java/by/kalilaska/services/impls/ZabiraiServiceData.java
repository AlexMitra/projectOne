...
public class ZabiraiServiceData implements ServiceOne{
	...
	public List<AccountBean> getSearchedAccounts(String part, String searchField, 
		...
		System.out.println("in getSearchedAccounts(), roles: " + roles + ", length: " + roles.length());
		...
		if(roles.length() == 0){
			...
			for (String role : rolesArr) {
				System.out.println("in getSearchedAccounts(), 2 if(), role: " + role);
				if(role.length()>=1){
					if(searchField.equalsIgnoreCase("byLogin")){
						if(searchPlace.equalsIgnoreCase("atFirstLetters")){
							List<AccountEntityHibernate> accountEntityList = accountsRepository.findByAccountLoginAndAccountRole("%" + part.trim(), role);
							//findByAccountLoginLike(part.trim() + "%");
							List<AccountBean> accountBeanList = entityToBeanConverter.convertToBeanList(
									accountEntityList, AccountBean.class);
							return accountBeanList;
						...
						if(searchPlace.equalsIgnoreCase("atFirstLetters")){
							List<AccountEntityHibernate> accountEntityList = accountsRepository.findByAccountEmailAndAccountRole("%" + part.trim(), role);
							//findByAccountEmailLike(part.trim() + "%");
							List<AccountBean> accountBeanList = entityToBeanConverter.convertToBeanList(
									accountEntityList, AccountBean.class);
							return accountBeanList;
