...
var adsTableDisableButton = {
	...
	showTitle: function(){
		if(this.baseTitle == null){
			this.baseTitle = document.getElementById("disable-ad-dialog-title").innerHTML;
		}

		if(WorkWithAdsTableElements.selectedAdsArr.length > 0){

			this.checkboxId = WorkWithAdsTableElements.selectedAdsArr[0];
			this.adId = this.checkboxId.replace("checkbox-ad-", "");

			var adName = document.getElementById("adName-" + this.adId).innerHTML;

			var element = document.getElementById("disable-ad-dialog-title");
	...
	disableAd: function(){
		...
		$.ajax({
			...
			success : function(data, textStatus, xhr) {
				...
				document.getElementById("disable-ad-dialog-close").click();
